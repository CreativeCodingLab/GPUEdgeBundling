...
float computeDividedLength() {
    float len = 0.0;
    vec3 P1, P2;
    P1 = texture2D(edges, vec2((0.+cOffset)/(W-1.), r)).xyz; // first point
    // total points is oldP + 2: 0 to oldP+1
    for (float i=1.; i < 1000.; i++) { // exclude first point
        if (i > oldP+1.) // last point
            break;
        P2 = texture2D(edges, vec2((i+cOffset)/(W-1.), r)).xyz;;
        len += distance(P1, P2);
        P1 = P2;
    }
    return len;
...
void main(void) {
    ...
    if (isEndPoint && isInScope) {
        ...
        outP = texture2D(edges, vec2((max(0., floor(float(cOut) - oldP))+cOffset)/(W-1.), r)).xyz;
    } else if (isInScope) {
        int c = 1;
        bool done = false;
        ...
        vec3 P1 = texture2D(edges, vec2((0.+cOffset)/(W-1.), r)).xyz, P2;
